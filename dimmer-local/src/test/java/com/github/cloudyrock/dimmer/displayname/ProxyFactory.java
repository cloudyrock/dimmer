...
class ProxyFactory {
  ...
  private static Class<?> createProxyClass(Class<?> mockedType) {
    ...
    enhancer.setNamingPolicy(DimmerTestProxyNamingPolicy.INSTANCE);
  ...
  <T> T createProxyFromOriginal(final T original) {
    return createProxyFromOriginal(original, preInterceptor);
  }

  @SuppressWarnings("unchecked")
  <T> T createProxyFromOriginal(final T original, PreInterceptor preInterceptor) {
    try {
  ...
  private static class DimmerTestProxyNamingPolicy extends DefaultNamingPolicy {
    static final DimmerTestProxyNamingPolicy INSTANCE = new DimmerTestProxyNamingPolicy();

    private DimmerTestProxyNamingPolicy() {
    }
